<div class="space-y-6">
    ...
    <div class="flex items-center gap-4 bg-[#1E1B4B] p-5 md:p-6 rounded-3xl border border-[#2E2A68] shadow-lg">
        <a href="{{ route('history.index') }}" class="w-10 h-10 rounded-2xl bg-[#16192E] hover:bg-[#25215A] text-slate-300 flex items-center justify-center font-bold text-sm border border-[#2E2A68] transition shrink-0">
            ...
            <div class="bg-[#1E1B4B] p-4 rounded-3xl border border-[#2E2A68] shadow-xl space-y-3">
                <div class="flex gap-2">
                    <div class="relative flex-1">
                        ...
                        <input type="text" wire:model.live.debounce.250ms="search" placeholder="Cari produk (Matcha, Kopi, Cendol)..." class="w-full pl-10 pr-3 py-2.5 bg-[#16192E] border border-[#2E2A68] rounded-xl text-xs text-white placeholder-slate-400 focus:outline-none focus:border-[#4338CA]" />
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[52vh] overflow-y-auto pr-1">
                    @forelse($products as $p)
                        <button wire:click="addToCart('{{ $p->id }}')" class="text-left bg-[#16192E] hover:bg-[#25215A] border border-[#2E2A68] hover:border-[#4338CA] rounded-2xl p-3 transition group active:scale-95">
            ...
            <div class="bg-[#1E1B4B] p-5 rounded-3xl border border-[#2E2A68] shadow-xl space-y-4">
                        ...
                        @foreach($cart as $idx => $item)
                            <div class="flex items-center gap-2 bg-[#16192E] border border-[#2E2A68] rounded-2xl p-3">
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <button wire:click="incrementQty({{ $idx }})" class="w-7 h-7 rounded-xl bg-[#4338CA] text-white font-black text-xs">+</button>
                                </div>
                ...
                <div class="grid grid-cols-2 gap-3 pt-2 border-t border-[#2E2A68]">
                    <div>
                        ...
                        <input type="date" wire:model="transactionDate" class="w-full px-3 py-2.5 bg-[#16192E] border border-[#2E2A68] rounded-xl text-xs text-white font-bold focus:outline-none focus:border-[#4338CA]">
                    ...
                    <div>
                        ...
                        <input type="time" wire:model="transactionTime" class="w-full px-3 py-2.5 bg-[#16192E] border border-[#2E2A68] rounded-xl text-xs text-white font-bold focus:outline-none focus:border-[#4338CA]">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        @foreach(['CASH'=>'Tunai','QRIS'=>'QRIS','GOFOOD'=>'GoFood','GRABFOOD'=>'Grab','SHOPEEFOOD'=>'Shopee'] as $val=>$label)
                            <button type="button" wire:click="$set('paymentMethod','{{ $val }}')" class="py-2.5 rounded-xl border font-bold text-xs transition {{ $paymentMethod===$val ? 'bg-[#4338CA] text-white border-[#4338CA]' : 'bg-[#16192E] text-slate-400 border-[#2E2A68] hover:text-white' }}">{{ $label }}</button>
                        @endforeach
                    </div>
                </div>

                <label class="flex items-center gap-2 text-xs font-bold text-slate-300 cursor-pointer">
                    <input type="checkbox" wire:model.live="affectStock" class="rounded border-[#2E2A68] bg-[#16192E] text-[#4338CA]">
                ...
                <button wire:click="saveTransaction" @if(empty($cart)) disabled @endif class="w-full py-4 bg-[#4338CA] hover:bg-[#3730A3] disabled:bg-slate-700 disabled:text-slate-400 text-white font-extrabold text-xs uppercase tracking-wider rounded-2xl shadow-xl transition active:scale-95 flex items-center justify-center gap-2">
