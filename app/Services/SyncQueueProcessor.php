...
class SyncQueueProcessor
{
    ...
    {
        if ($entry->processed_at) return 'SYNCED';
        try {
    ...
    private function member(array $payload): void
    {
        ...
        \App\Models\LoyaltyMember::firstOrCreate(['id'=>$payload['client_member_id']], collect($payload)->except('client_member_id')->toArray());
    }
    ...
    private function stamp(array $payload): void
    {
        ...
        if (\App\Models\LoyaltyStamp::whereKey($payload['client_stamp_id'])->exists()) return;
        $member=\App\Models\LoyaltyMember::lockForUpdate()->findOrFail($payload['loyalty_member_id']);
        \App\Models\LoyaltyStamp::create(['id'=>$payload['client_stamp_id'],'loyalty_member_id'=>$member->id,'program_id'=>$payload['program_id'],'transaction_id'=>$payload['transaction_id']??null,'stamps_earned'=>$payload['stamps_earned'],'stamped_at'=>$payload['stamped_at']]);
        $member->increment('stamps_count',(int)$payload['stamps_earned']); $member->increment('total_visits');
    }
    ...
    private function reward(array $payload): void
    {
        ...
        \App\Models\CustomerReward::firstOrCreate(['id'=>$payload['client_reward_id']], collect($payload)->except('client_reward_id')->toArray());
    }

    private function expense(array $payload): void
    {
        $payload['client_expense_id'] = $payload['client_expense_id'] ?? $payload['entity_id'] ?? (string) \Illuminate\Support\Str::uuid();
    ...
    private function transaction(array $payload): void
    {
        ...
        if (!$transaction->wasRecentlyCreated) return;
