...
class SqliteRepository
{
    ...
    {
        return SqliteConnection::pdo($this->dbPath);
    }
    ...
    public function catalogList(?string $categoryId = null, ?string $search = null): array
    {
        ...
        if ($categoryId && $categoryId !== 'ALL') {
            $sql .= ' AND id IN (SELECT id FROM catalog WHERE 1)';
        }
    ...
    {
        $st = $this->pdo()->prepare('SELECT * FROM catalog WHERE id=:id');
        $st->execute(['id' => $id]);
    ...
    {
        $st = $this->pdo()->prepare('SELECT * FROM catalog WHERE sku=:sku');
        $st->execute(['sku' => $sku]);
        $r = $st->fetch(PDO::FETCH_ASSOC);
    ...
    public function stockSnapshot(): array
    {
        ...
        $out = [];
        foreach ($rows as $r) {
            $out[$r['id']] = (float) $r['current_stock'];
        }
...
return $out;
    ...
    {
        $pdo = $this->pdo();
        $st = $pdo->prepare('INSERT INTO catalog(id,name,sku,price,hpp,current_stock,is_active,updated_at) VALUES(:id,:name,:sku,:price,:hpp,:current_stock,:is_active,:updated_at) ON CONFLICT(id) DO UPDATE SET name=excluded.name, sku=excluded.sku, price=excluded.price, hpp=excluded.hpp, current_stock=excluded.current_stock, is_active=excluded.is_active, updated_at=excluded.updated_at');
        foreach ($products as $p) {
            $st->execute(['id' => $p['id'], 'name' => $p['name'], 'sku' => $p['sku'] ?? null, 'price' => $p['price'] ?? 0, 'hpp' => $p['hpp'] ?? 0, 'current_stock' => $p['current_stock'] ?? 0, 'is_active' => (int) ($p['is_active'] ?? 1), 'updated_at' => $p['updated_at'] ?? $this->now()]);
        }
    }
    ...
    {
        $pdo = $this->pdo();
        $st = $pdo->prepare('INSERT INTO members(id,name,phone,qr_code,status,email,stamps_count,total_visits,updated_at) VALUES(:id,:name,:phone,:qr_code,:status,:email,:stamps_count,:total_visits,:updated_at) ON CONFLICT(id) DO UPDATE SET name=excluded.name, phone=excluded.phone, qr_code=excluded.qr_code, status=excluded.status, email=excluded.email, stamps_count=excluded.stamps_count, total_visits=excluded.total_visits, updated_at=excluded.updated_at');
        foreach ($members as $m) {
            $st->execute(['id' => $m['id'], 'name' => $m['name'] ?? null, 'phone' => $m['phone'] ?? null, 'qr_code' => $m['qr_code'] ?? null, 'status' => $m['status'] ?? 'ASSIGNED', 'email' => $m['email'] ?? null, 'stamps_count' => $m['stamps_count'] ?? 0, 'total_visits' => $m['total_visits'] ?? 0, 'updated_at' => $m['updated_at'] ?? $this->now()]);
        }
    }
    ...
    {
        $st = $this->pdo()->prepare('SELECT payload FROM carts WHERE id=:id');
        $st->execute(['id' => 'carts']);
        $r = $st->fetch(PDO::FETCH_ASSOC);
        if (! $r) {
            return [];
    ...
    {
        $carts = $this->getMultiCart();
        $idx = $this->getActiveCartIndex();

        return $carts[$idx]['items'] ?? [];
    }

    public function setActiveCartIndex(int $index): void
    {
        $pdo = $this->pdo();
        $st = $pdo->prepare('SELECT payload FROM carts WHERE id=:id');
        $st->execute(['id' => 'carts']);
        $r = $st->fetch(PDO::FETCH_ASSOC);
        $d = $r ? json_decode($r['payload'], true) : ['items' => []];
        $d['activeCartId'] = $index;
        $pdo->prepare('INSERT INTO carts(id,payload,updated_at) VALUES(:id,:payload,:updated_at) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload, updated_at=excluded.updated_at')->execute(['id' => 'carts', 'payload' => json_encode($d), 'updated_at' => $this->now()]);
    }
    ...
    public function getActiveCartIndex(): int
    {
        ...
        return (int) ($d['activeCartId'] ?? $d['active_cart'] ?? 0);
    }
    ...
    public function addItem(array $item): void
    {
        ...
        $idx = $this->getActiveCartIndex();
        if (! isset($carts[$idx])) {
            $carts[$idx] = ['name' => 'Cart '.($idx + 1), 'items' => []];
        } $carts[$idx]['items'][] = $item;
    ...
    public function updateQuantity(string $itemKey, int $qty): void
    {
        ...
        if (! isset($carts[$idx])) {
            return;
        } foreach ($carts[$idx]['items'] as $k => &$it) {
            if (($it['id'] ?? null) === $itemKey) {
                if ($qty <= 0) {
                    unset($carts[$idx]['items'][$k]);
                } else {
                    $it['qty'] = $qty;
                } break;
            }
        } $carts[$idx]['items'] = array_values($carts[$idx]['items']);
        $this->saveCarts($carts);
    }

    public function removeItem(string $itemKey): void
    {
        $this->updateQuantity($itemKey, 0);
    }
    ...
    {
        $pdo = $this->pdo();
        $pdo->prepare('INSERT INTO carts(id,payload,updated_at) VALUES(:id,:payload,:updated_at) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload, updated_at=excluded.updated_at')->execute(['id' => 'carts', 'payload' => json_encode(['items' => $carts, 'activeCartId' => $this->getActiveCartIndex()]), 'updated_at' => $this->now()]);
    }

    public function createNewCart(?string $name = null): void
    {
        $carts = $this->getMultiCart();
        $carts[] = ['name' => $name ?: ('Cart '.(count($carts) + 1)), 'items' => []];
        $this->saveCarts($carts);
    }
    ...
    public function createTransaction(array $data, array $items): string
    {
        ...
        try {
            ...
            foreach ($items as $it) {
                $st->execute(['id' => (string) Str::uuid(), 'transaction_id' => $txId, 'product_id' => $it['product_id'] ?? $it['id'] ?? null, 'product_name' => $it['product_name'] ?? $it['name'] ?? '', 'unit_price' => $it['unit_price'] ?? $it['price'] ?? 0, 'unit_hpp' => $it['unit_hpp'] ?? $it['hpp'] ?? 0, 'quantity' => $it['quantity'] ?? $it['qty'] ?? 1, 'subtotal' => $it['subtotal'] ?? 0]);
            }
            ...
            foreach ($items as $it) {
                ...
                if ($pid) {
                    $pdo->prepare('UPDATE catalog SET current_stock = current_stock - :qty WHERE id=:id')->execute(['qty' => $qty, 'id' => $pid]);
                }
            }
    ...
    {
        $st = $this->pdo()->prepare('SELECT * FROM transactions_local WHERE id=:id');
        $st->execute(['id' => $id]);
        $r = $st->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $st = $this->pdo()->prepare('SELECT * FROM members WHERE qr_code=:qr LIMIT 1');
        $st->execute(['qr' => $qr]);
        $r = $st->fetch(PDO::FETCH_ASSOC);
        if ($r) {
            return $r;
        ...
        $st = $this->pdo()->prepare('SELECT * FROM members WHERE qr_code LIKE :q LIMIT 1');
        $st->execute(['q' => '%'.$suffix]);
    ...
    {
        $st = $this->pdo()->prepare('SELECT * FROM members WHERE phone=:p LIMIT 1');
        $st->execute(['p' => $phone]);
