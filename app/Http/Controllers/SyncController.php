...
class SyncController extends Controller
{
    ...
    public function push(Request $request, SyncQueueProcessor $processor)
    {
        ...
        $device=SyncDevice::whereKey($data['device_id'])->where('user_id',$request->user()->id)->firstOrFail(); $results=[];
        ...
        $device->update(['last_synced_at'=>now()]); return response()->json(['results'=>$results,'cursor'=>now()->toISOString()]);
    }
    public function pull(Request $request)
    {
        ...
        $products = \App\Models\Product::query()->when($cursor, fn($q) => $q->where('updated_at', '>', $cursor))->get()->map(fn($p) => ['id'=>$p->id,'name'=>$p->name,'sku'=>$p->sku,'price'=>$p->price,'hpp'=>$p->hpp,'current_stock'=>$p->current_stock,'is_active'=>$p->is_active,'updated_at'=>$p->updated_at?->toISOString()])->values();
        $programs = \App\Models\LoyaltyProgram::query()->when($cursor, fn($q) => $q->where('updated_at', '>', $cursor))->get()->map(fn($p) => ['id'=>$p->id,'name'=>$p->name,'target_stamps'=>$p->target_stamps,'min_transaction'=>$p->min_transaction,'is_active'=>$p->is_active,'updated_at'=>$p->updated_at?->toISOString()])->values();
